...
export default function Table({ data }: { data: matchesDataType }) {
  return (
    <div className="py-3 px-2 md:px-3 rounded-md flex flex-col gap-3 bg-neutral-700">
      {data?.matches?.map((match: matchesType) => {
        const d = new Date(match?.utcDate);
        return (
          <>
            {/* Header */}
            <div className=" my-2 flex justify-between items-center px-4 py-1 bg-zinc-600 rounded-lg">
